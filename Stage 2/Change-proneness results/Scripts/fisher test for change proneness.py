...
def process_csv(file_path, output_path=None, alternative='greater'):
    ...
    for index, row in df.iterrows():
        ...
        a = pd.to_numeric(row.get("Both Smelly & Changed"), errors='coerce') or 0
        b = pd.to_numeric(row.get("Smelly But Not Changed"), errors='coerce') or 0
        c = pd.to_numeric(row.get("Faulty But Not Changed"), errors='coerce') or 0
        d = pd.to_numeric(row.get("Not Smelly & Not Changed"), errors='coerce') or 0
...
if __name__ == "__main__":
    csv_file = "buggy_smelly/abidi/fisher_data_for_change_analysis.csv"   #path to csv file which contains the contingency table for fisher test
    output_file = "buggy_smelly/abidi/fisher_results_for_change_analysis.csv"  # path to outpue
